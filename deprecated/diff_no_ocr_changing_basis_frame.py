...
def align_frames_multiprocessing(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    num_processes = mp.cpu_count()
    p = mp.Pool(num_processes)
    args = [[basis_frame, frame, basis_frame_shift] for frame in frames]
...
def align_frames(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    for frame in frames:
        aligned_frame, shift = align_frame(basis_frame, frame, basis_frame_shift)
        aligned_frames.append(aligned_frame)
        shifts.append(shift)
    return aligned_frames, shifts

def align_frame(frame1, frame2, basis_frame_shift):
...
def align_frames_integer_shift(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    for frame in frames:
        aligned_frame, shift = align_frame_integer_shift(basis_frame, frame, basis_frame_shift)
        aligned_frames.append(aligned_frame)
        shifts.append(shift)
    return aligned_frames, shifts

def align_frame_integer_shift(frame1, frame2, basis_frame_shift):
    # Convert images to grayscale
    frame1_gray = cv2.cvtColor(frame1,cv2.COLOR_BGR2GRAY)
    frame2_gray = cv2.cvtColor(frame2,cv2.COLOR_BGR2GRAY)
...
def align_frames_orb_multiprocessing(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    num_processes = mp.cpu_count()
    p = mp.Pool(num_processes)
...
def align_frames_orb(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    for frame in frames:
        aligned_frame, shift = align_frame_orb(basis_frame, frame, basis_frame_shift)
        aligned_frames.append(aligned_frame)
        shifts.append(shift)
    return aligned_frames, shifts

def align_frame_orb(frame1, frame2, basis_frame_shift):
    # Convert images to grayscale
    frame1_gray = cv2.cvtColor(frame1,cv2.COLOR_BGR2GRAY)
    frame2_gray = cv2.cvtColor(frame2,cv2.COLOR_BGR2GRAY)
...
def align_frames_integer_shift_orb(basis_frame, frames, basis_frame_shift):
    aligned_frames = []
    shifts = []
    for frame in frames:
        aligned_frame, shift = align_frame_integer_shift_orb(basis_frame, frame, basis_frame_shift)
        aligned_frames.append(aligned_frame)
        shifts.append(shift)
    return aligned_frames, shifts

def align_frame_integer_shift_orb(frame1, frame2, basis_frame_shift):
    # Convert images to grayscale
    frame1_gray = cv2.cvtColor(frame1,cv2.COLOR_BGR2GRAY)
    frame2_gray = cv2.cvtColor(frame2,cv2.COLOR_BGR2GRAY)
...
# フレーム数が大きいと基準画像からのずれが大きくなるため、基準画像を10フレームごとに変化する。
# ステージのずれ評価専用
if __name__ == "__main__":
    ...
    for group in range(int(len(frames)/group_frame_count)):
        print(f"group {group}")
        # グループに属するフレーム
        group_frames = frames[group*group_frame_count: (group+1)*group_frame_count]
        # 基準画像とその他に分割
        group_aligned_frames, group_shifts = align_frames_orb(basis_frame, group_frames, basis_frame_shift) # 基準・測定画像の位置合わせ
